...
export const MessagesTable = ({ reload }) => {
  ...
  useEffect(() => {
    ...
    Axios({
      method: "GET",
      url: `${process.env.REACT_APP_PARTNER_API}/messages`,
      headers: {
  ...
  const TableRow = (props) => {
    const { ID, user_group, message, phones, created_at } = props;
    ...
    return (
      <tr>
        ...
        <td>
          <span className="fw-normal">{user_group}</span>
        </td>
        <td>
          <span className="fw-normal">{message}</span>
        </td>
        <td>
          <span className="fw-normal">{phones.split(",").length}</span>
        </td>
        <td>
        ...
        <Table hover className="user-table align-items-center" size="sm">
          ...
          <tbody>
            ...
            {loadingStatus ? (
              ...
              messages.map((t) => <TableRow key={`msg-${t.ID}`} {...t} />)
            )}
          </tbody>
        </Table>
        {!loadingStatus && (

...
export const SubCategoryTable = ({ reload, setReload }) => {
  let { id } = useParams();
      ...
      <Card.Body className="pt-0">
        <Table hover className="user-table align-items-center" size="sm">
          ...
          <tbody>
            ...
            {loadingStatus && categories.sub_categories === undefined ? (
              <Spinner animation="border" />
            ) : (
              categories.sub_categories !== undefined &&
              categories.sub_categories.map((t) => (
                <TableRow key={`cat-${t.ID}`} {...t} />
              ))
            )}
          </tbody>
        </Table>
